...
console.log('Starting task-manager-ai...');
...
function copyTemplateFile(templateName, targetPath, replacements = {}) {
  ...
  switch(templateName) {
    ...
    case 'self_improve.mdc':
      ...
      break;
    case 'README-task-manager.md':
      sourcePath = path.join(__dirname, '..', 'README-task-manager.md');
      break;
    case 'windsurfrules':
  ...
  if (fs.existsSync(targetPath)) {
    ...
    if (filename === 'README.md') {
      ...
      const taskMasterReadmePath = path.join(path.dirname(targetPath), 'README-task-manager.md');
...
function createProjectStructure(projectName, projectDescription, projectVersion, authorName, skipInstall) {
  ...
  copyTemplateFile('README-task-manager.md', path.join(targetDir, 'README.md'), replacements);
  ...
  console.log(boxen(
    ...
    chalk.white('   └─ ') + chalk.dim('You can also run ') + chalk.cyan('task-manager parse-prd <your-prd-file.txt>') + '\n' +
  ...
  try {
    ...
    if (options.yes || (options.name && options.description)) {
      ...
      await initializeProject({
        projectName: options.name || 'task-manager-project',
