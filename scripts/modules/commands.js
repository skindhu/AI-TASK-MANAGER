...
function registerCommands(programInstance) {
    ...
    .action(async (file, options) => {
      ...
      if (!inputFile) {
        if (fs.existsSync(defaultPrdPath)) {
          ...
          await parsePRD(defaultPrdPath, outputPath, numTasks, knowledgeBasePath);
          return;
        }
  ...
  programInstance
    ...
    .option('-k, --knowledge-base <path>', 'Path to business knowledge documents to use as context (will use metadata from parse-prd if not specified)')
    .action(async (options) => {
      const tasksPath = options.file;
  ...
  programInstance
    .command('server')
    .description('Start the task management web UI server')
