 ...
 */
async function parsePRD(prdPath, tasksPath, numTasks, knowledgeBasePath = null) {
  try {
    ...
    // Process knowledge base documents if provided
    let knowledgeBase = null;
    if (knowledgeBasePath && fs.existsSync(knowledgeBasePath)) {
      log('info', `Reading business knowledge from: ${knowledgeBasePath}`);
      knowledgeBase = readKnowledgeBase(knowledgeBasePath);
    }

    // Call Gemini to generate tasks
    const tasksData = await callGemini(prdContent, prdPath, numTasks, 0, knowledgeBase);

    // Save knowledgeBasePath to metadata if provided
    if (knowledgeBasePath) {
      tasksData.metadata = tasksData.metadata || {};
      tasksData.metadata.knowledgeBasePath = knowledgeBasePath;
      log('info', `Saved knowledge base path to metadata: ${knowledgeBasePath}`);
    }
...
/**
 * Read and organize business knowledge documents from a directory
 * @param {string} knowledgeBasePath - Path to the knowledge base directory
 * @returns {string} Formatted knowledge base content
 */
function readKnowledgeBase(knowledgeBasePath) {
  try {
    let knowledgeContent = '';

    // Check if knowledgeBasePath is a directory or a file
    ...
    if (stats.isDirectory()) {
      ...
      const files = fs.readdirSync(knowledgeBasePath)
        .filter(file => {
          ...
          return ['.txt', '.md', '.json', '.yaml', '.yml', '.html', '.csv', '.js', '.ts'].includes(ext);
        })
      ...
      knowledgeContent = `\n\n### DOCUMENT: ${path.basename(knowledgeBasePath)}\n\n${content}`;
    }

    if (!knowledgeContent) {
      log('warn', `No valid knowledge documents found in ${knowledgeBasePath}`);
      return null;
    }

    return `## BUSINESS KNOWLEDGE CONTEXT\n${knowledgeContent}\n\n## END OF BUSINESS KNOWLEDGE CONTEXT`;
  } catch (error) {
    log('warn', `Failed to read knowledge base: ${error.message}`);
    return null;
  }
}
...
async function updateTasks(tasksPath, fromId, prompt, useResearch = false) {
  try {
    ...
    if (useResearch && (!perplexity || !process.env.PERPLEXITY_API_KEY)) {
      log('warn', 'Perplexity AI is not available. Falling back to Gemini AI.');
      console.log(chalk.yellow('Perplexity AI is not available (API key may be missing). Falling back to Gemini AI.'));
      useResearch = false;
    }
...
async function expandTask(taskId, numSubtasks = CONFIG.defaultSubtasks, useResearch = false, additionalContext = '', knowledgeBasePath = null) {
  try {
    ...
    // If no knowledgeBasePath is provided, check if it's stored in metadata
    if (!knowledgeBasePath && data.metadata && data.metadata.knowledgeBasePath) {
      knowledgeBasePath = data.metadata.knowledgeBasePath;
      log('info', `Using knowledge base path from metadata: ${knowledgeBasePath}`);
      console.log(chalk.blue(`Using business knowledge from metadata: ${knowledgeBasePath}`));
    }
    ...
    if (useResearch) {
      ...
      subtasks = await generateSubtasksWithPerplexity(task, numSubtasks, nextSubtaskId, additionalContext, knowledgeBase);
    } else {
      log('info', 'Generating subtasks with Gemini only');
      subtasks = await generateSubtasks(task, numSubtasks, nextSubtaskId, additionalContext, knowledgeBase);
    }
 ...
 * @param {string} knowledgeBasePath - Optional path to business knowledge documents
 */
async function expandAllTasks(numSubtasks = CONFIG.defaultSubtasks, useResearch = false, additionalContext = '', forceFlag = false, knowledgeBasePath = null) {
  try {
    displayBanner();
    ...
    const complexityReport = readComplexityReport();
    log('info', complexityReport ? 'Using complexity report for task prioritization' : 'No complexity report found');

    // Filter for pending tasks
    const pendingTasks = data.tasks.filter(t =>
      t.status === 'pending' &&
      (!t.subtasks || t.subtasks.length === 0 || forceFlag)
    );

    if (pendingTasks.length === 0) {
      const message = forceFlag ?
        'No pending tasks found' :
        'No pending tasks without subtasks found. Use --force to regenerate subtasks for all pending tasks.';
      console.log(chalk.yellow(message));
      return;
    }

    console.log(boxen(
      chalk.white.bold(`Expanding ${pendingTasks.length} Pending Tasks`),
      { padding: 1, borderColor: 'blue', borderStyle: 'round', margin: { top: 0, bottom: 1 } }
    ));

    // Clone the pending tasks array for prioritization
    ...
    if (complexityReport && complexityReport.complexityAnalysis) {
      ...
      log('info', 'Tasks sorted by complexity score (high to low)');
    } else {
      log('info', 'Sorting tasks by ID...');
      tasksToExpand.sort((a, b) => a.id - b.id);
    }

    // Display the tasks to be expanded
    console.log(chalk.bold('Tasks to be expanded:'));
    ...
    for (const task of tasksToExpand) {
      // Skip tasks with subtasks unless force flag is set
      if (task.subtasks && task.subtasks.length > 0 && !forceFlag) {
        log('info', `Skipping task ${task.id} (already has ${task.subtasks.length} subtasks)`);
        console.log(chalk.yellow(`Skipping task ${task.id} (already has ${task.subtasks.length} subtasks)`));
        continue;
      }

      // Find task complexity analysis if available
      let taskAnalysis = null;
      if (complexityReport && complexityReport.complexityAnalysis) {
        taskAnalysis = complexityReport.complexityAnalysis.find(a => a.taskId === task.id);
      }

      // Use recommended subtask count if available, otherwise use the default
      let taskSubtaskCount = numSubtasks;
      if (taskAnalysis && taskAnalysis.recommendedSubtasks) {
        taskSubtaskCount = taskAnalysis.recommendedSubtasks;
        log('info', `Using recommended subtask count for task ${task.id}: ${taskSubtaskCount}`);
      }

      // Use task-specific expansion prompt if available
      let taskContext = additionalContext;
      if (taskAnalysis && taskAnalysis.expansionPrompt) {
        if (additionalContext) {
          taskContext = `${additionalContext}\n\nTask-specific guidance: ${taskAnalysis.expansionPrompt}`;
        } else {
          taskContext = taskAnalysis.expansionPrompt;
        }
        log('info', `Using task-specific expansion prompt for task ${task.id}`);
      }

      console.log(chalk.bold(`\nExpanding task ${task.id}: ${task.title}...\n`));

      // Clear existing subtasks if force flag is set
      if (task.subtasks && task.subtasks.length > 0 && forceFlag) {
        task.subtasks = [];
        log('info', `Cleared existing subtasks for task ${task.id}`);
      }

      // Call expandTask for this task
      await expandTask(task.id, taskSubtaskCount, useResearch, taskContext, knowledgeBasePath);
    }

    // Display summary
    console.log(boxen(
      chalk.green.bold(`Task Expansion Summary:`) + '\n\n' +
      chalk.white(`Expanded ${tasksToExpand.length} tasks with${useResearch ? ' research-backed' : ''} subtasks`),
...
DO NOT include any text before or after the JSON array. No explanations, no markdown formatting.`;
          ...
          console.log(chalk.yellow('Falling back to Gemini for complexity analysis...'));
...
DO NOT include any text before or after the JSON array. No explanations, no markdown formatting.`;
              ...
              // Use Gemini with non-streaming API
              const { geminiModel } = await import('./ai-services.js');
              ...
              try {
                ...
                // Get text from response
                missingAnalysisResponse = result.response.text();

                console.log(chalk.green("Completed non-streaming response from Gemini API!"));
              } catch (error) {
                clearInterval(animationInterval);
                throw error;
              }
